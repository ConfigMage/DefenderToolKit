...
function Toggle-DarkMode {
    ...
    foreach ($button in @($connectButton, $refreshAllButton, $exportReportButton, $viewLogButton, 
                         ...
                         $cfaStatusButton, $tamperProtectionButton, $updateStatusButton,
                         $protectionHistoryButton)) {
        if ($button.Enabled) {
...
function Enable-OperationButtons {
    ...
    $updateStatusButton.Enabled = $Enable
    $exportReportButton.Enabled = $Enable
    $refreshAllButton.Enabled = $Enable
    $protectionHistoryButton.Enabled = $Enable
}
...
function Get-ProtectionHistory {
    if (-not $script:isConnected) {
        Show-ErrorMessage "Not connected to any computer"
        return
    }
    ...
    try {
        Append-Output "`n=== Recent Protection History ===" -Color ([System.Drawing.Color]::DarkBlue)
        
        $protectionHistory = Invoke-RemoteCommand -ComputerName $script:currentComputer -Credential $script:credential -ScriptBlock {
            # Get threat history from the last 30 days
            $startDate = (Get-Date).AddDays(-30)
            
            # Query Windows Defender events
            $events = @()
            
            # Get threat detection events (Event ID 1116, 1117)
            $threatEvents = Get-WinEvent -FilterHashtable @{LogName='Microsoft-Windows-Windows Defender/Operational'; ID=1116,1117; StartTime=$startDate} -ErrorAction SilentlyContinue
            
            # Get real-time protection events (Event ID 1000, 1001)
            $realtimeEvents = Get-WinEvent -FilterHashtable @{LogName='Microsoft-Windows-Windows Defender/Operational'; ID=1000,1001; StartTime=$startDate} -ErrorAction SilentlyContinue
            
            # Get action events (Event ID 1118, 1119)
            $actionEvents = Get-WinEvent -FilterHashtable @{LogName='Microsoft-Windows-Windows Defender/Operational'; ID=1118,1119; StartTime=$startDate} -ErrorAction SilentlyContinue
            
            # Combine all events
            $allEvents = @()
            if ($threatEvents) { $allEvents += $threatEvents }
            if ($realtimeEvents) { $allEvents += $realtimeEvents }
            if ($actionEvents) { $allEvents += $actionEvents }
            
            # Process events
            $processedEvents = $allEvents | Sort-Object TimeCreated -Descending | Select-Object -First 50 | ForEach-Object {
                ...
                [PSCustomObject]@{
                    ...
                    ThreatName = $eventData.'Threat Name'
                    Path = $eventData.Path
                    Process = $eventData.'Process Name'
                    Action = $eventData.'Action Name'
                    User = $eventData.'Detection User'
                    Message = $_.Message
                }
            }
            
            # Also get threat history from Get-MpThreatDetection
            $mpThreats = Get-MpThreatDetection -ErrorAction SilentlyContinue | Where-Object { $_.InitialDetectionTime -gt $startDate }
        ...
        if ($protectionHistory.Threats) {
            Append-Output "`nRecent Threat Detections:" -Color ([System.Drawing.Color]::DarkGreen)
            foreach ($threat in $protectionHistory.Threats) {
                $color = switch ($threat.ThreatStatusID) {
                    2 { [System.Drawing.Color]::Green }  # Cleaned
                    3 { [System.Drawing.Color]::Orange } # Quarantined
                    6 { [System.Drawing.Color]::Red }    # Failed
                    default { [System.Drawing.Color]::Black }
                }
                
                Append-Output "`nDetection Time: $($threat.InitialDetectionTime)"
                Append-Output "Threat: $($threat.ThreatName)" -Color ([System.Drawing.Color]::Red)
                Append-Output "Status: $($threat.ThreatStatusID)" -Color $color
                Append-Output "Action: $($threat.CleaningActionID)"
                if ($threat.Resources) {
                    Append-Output "Resources affected:"
            ...
            Append-Output "`nNo recent threat detections found (good news!)" -Color ([System.Drawing.Color]::Green)
        }
        
        if ($protectionHistory.Events) {
            Append-Output "`n`nRecent Protection Events:" -Color ([System.Drawing.Color]::DarkGreen)
            $groupedEvents = $protectionHistory.Events | Group-Object EventID
            ...
            foreach ($group in $groupedEvents) {
                $eventType = switch ($group.Name) {
                    ...
                    "1117" { "Windows Defender took action to protect from malware" }
                    "1118" { "Windows Defender failed to take action" }
                    "1119" { "Windows Defender encountered critical error" }
                    "1000" { "Real-time protection scan started" }
                    "1001" { "Real-time protection scan finished" }
                    default { "Event ID $($group.Name)" }
                }
                
                Append-Output "`n$eventType (Count: $($group.Count))" -Color ([System.Drawing.Color]::DarkBlue)
                
                # Show first 5 events of each type
                $group.Group | Select-Object -First 5 | ForEach-Object {
                    Append-Output "  Time: $($_.Time)"
                    if ($_.ThreatName) { Append-Output "  Threat: $($_.ThreatName)" -Color ([System.Drawing.Color]::Red) }
                    if ($_.Path) { Append-Output "  Path: $($_.Path)" }
                    if ($_.Action) { Append-Output "  Action: $($_.Action)" }
                    Append-Output ""
                }
            }
        }
        
        Write-Log "Protection history retrieved successfully"
    }
    catch {
        $errorMessage = "Failed to get protection history: $($_.Exception.Message)"
        Append-Output $errorMessage -Color ([System.Drawing.Color]::Red)
        Write-Log $errorMessage -Level "ERROR"
    }
}
...
function Check-UpdateStatus {
    ...
    try {
        Append-Output "`n=== Update Status ===" -Color ([System.Drawing.Color]::DarkBlue)
        
        $updateStatus = Invoke-RemoteCommand -ComputerName $script:currentComputer -Credential $script:credential -ScriptBlock {
        ...
        Append-Output "Engine Version: $($updateStatus.AMEngineVersion)"
        Append-Output "Last Signature Update: $($updateStatus.AntivirusSignatureLastUpdated)"
        Append-Output "Antivirus Signature Version: $($updateStatus.AntivirusSignatureVersion)"
        Append-Output "Antispyware Signature Version: $($updateStatus.AntispywareSignatureVersion)"
        
        Write-Log "Update status retrieved successfully"
    }
    catch {
        $errorMessage = "Failed to check update status: $($_.Exception.Message)"
        Append-Output $errorMessage -Color ([System.Drawing.Color]::Red)
        Write-Log $errorMessage -Level "ERROR"
    }
}
...
function Export-HTMLReport {
    ...
    try {
        ...
        $protectionHistory = Invoke-RemoteCommand -ComputerName $script:currentComputer -Credential $script:credential -ScriptBlock {
            ...
            # Get threat history
            $threats = Get-MpThreatDetection -ErrorAction SilentlyContinue | Where-Object { $_.InitialDetectionTime -gt $startDate }
            
            # Get recent events
            $events = Get-WinEvent -FilterHashtable @{LogName='Microsoft-Windows-Windows Defender/Operational'; ID=1116,1117,1118; StartTime=$startDate} -ErrorAction SilentlyContinue | Select-Object -First 20
...
<body>
    ...
    <div class="section">
        <h2>Protection History (Last 30 Days)</h2>
...
"@
            foreach ($threat in $protectionHistory.Threats | Select-Object -First 10) {
                $statusClass = switch ($threat.ThreatStatusID) {
                    2 { "status-good" }   # Cleaned
                    3 { "status-warning" } # Quarantined
                    6 { "status-bad" }    # Failed
                    default { "" }
                }
                ...
                $html += @"
            <tr>
                <td>$($threat.InitialDetectionTime)</td>
                <td>$($threat.ThreatName)</td>
                <td class="$statusClass">$($threat.ThreatStatusID)</td>
                <td>$($threat.CleaningActionID)</td>
            </tr>
"@
            }
            $html += @"
...
"@
            ...
            $html += @"
        <p class="status-good">No recent threats detected</p>
"@
        }
...
"@
            foreach ($event in $protectionHistory.Events | Select-Object -First 10) {
            ...
            <tr>
                ...
                <td>$eventType</td>
                <td>$($event.Message -replace '<[^>]+>','' | Select-Object -First 100)</td>
            </tr>
"@
            }
            $html += @"
...
$updateStatusButton = New-Object System.Windows.Forms.Button
$updateStatusButton.Text = "Update Status"
$updateStatusButton.Size = New-Object System.Drawing.Size(280, 40)
$updateStatusButton.Location = New-Object System.Drawing.Point(600, 80)
$updateStatusButton.BackColor = [System.Drawing.Color]::FromArgb(0, 51, 102)
$updateStatusButton.ForeColor = [System.Drawing.Color]::White
$updateStatusButton.Font = New-Object System.Drawing.Font("Arial", 10, [System.Drawing.FontStyle]::Bold)
$updateStatusButton.FlatStyle = [System.Windows.Forms.FlatStyle]::Flat
$updateStatusButton.Enabled = $false
$updateStatusButton.Add_Click({ Check-UpdateStatus })
$operationsGroupBox.Controls.Add($updateStatusButton)

# Admin functions row
$protectionHistoryButton = New-Object System.Windows.Forms.Button
$protectionHistoryButton.Text = "Protection History"
$protectionHistoryButton.Size = New-Object System.Drawing.Size(280, 40)
$protectionHistoryButton.Location = New-Object System.Drawing.Point(20, 130)
$protectionHistoryButton.BackColor = [System.Drawing.Color]::FromArgb(0, 51, 102)
$protectionHistoryButton.ForeColor = [System.Drawing.Color]::White
$protectionHistoryButton.Font = New-Object System.Drawing.Font("Arial", 10, [System.Drawing.FontStyle]::Bold)
$protectionHistoryButton.FlatStyle = [System.Windows.Forms.FlatStyle]::Flat
$protectionHistoryButton.Enabled = $false
$protectionHistoryButton.Add_Click({ Get-ProtectionHistory })
$operationsGroupBox.Controls.Add($protectionHistoryButton)
...
$clearOutputButton.Location = New-Object System.Drawing.Point(600, 130)
